...
def _expand_user_path(path: str) -> str:
    """Expand supported user-facing path shorthand before resolution."""
    return os.path.expandvars(_expand_tilde(path))
...
def _resolve_path_for_task(filepath: str, task_id: str = "default") -> Path:
    ...
    """
    p = Path(_expand_user_path(filepath))
    if p.is_absolute():
        return p.resolve()
...
def _path_resolution_warning(filepath: str, resolved: Path, task_id: str = "default") -> str | None:
    ...
    """
    try:
        if Path(_expand_user_path(filepath)).is_absolute():
            return None
        workspace_root = _authoritative_workspace_root(task_id)
        if not workspace_root:
            return None  # No authoritative workspace root to compare against.
        root = Path(_expand_user_path(workspace_root)).resolve()
